...
{
    // Normal Map    
    const float3 normalmap = 2.0f * (gTextures2048.Sample(gLinearSam, float3(_in.Tex, gNormalMapIndex)).rgb) - 1.0f;
    ...
    [branch]
    if(gUseTextures)
        texColor = gTextures2048.Sample(gLinearSam, float3(_in.Tex, gDiffuseMapIndex));
