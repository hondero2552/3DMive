...
LRESULT WinAppLayer::WinProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_RBUTTONDOWN:
        {           
            ...
            if(message == WM_LBUTTONDOWN)
            {
                ...
                // Get the quadrant
                uint quadrant = 0;
                if(mouse_x < m_iWidth/2)
                {
                    if(mouse_y > m_iHeight/2)
                        quadrant = 2;
                }
                else
                {
                ...
                if(pView->MouseClick(mouse_x, mouse_y, quadrant) != UIBUTTON::NO_BUTTON)
    ...
    case WM_LBUTTONUP:
    case WM_RBUTTONUP:
        if(message == WM_LBUTTONUP && mouseClickedUIButton)
        {
            mouse_x = GET_X_LPARAM(lParam);
            mouse_y = GET_Y_LPARAM(lParam);           
            ...
            const UIBUTTON button = pView->MouseButtonUp(mouse_x, mouse_y, quadrant);
            switch(button)
            {            
            case UIBUTTON::FLIPFACES:
                {
                    ...
                    if(pView->GetTriangleCount()>0)
                    {
                        ...
                        {
                            m_pIView->PrintMessage(
                                UI_MSG_TYPE::CRITICAL_ERROR, 
                                L"Mesh is not flippable because it does not have the same amount of normals and vertices."
                                );
                        }
                    }
                    else
    ...
    case WM_MOUSEMOVE:
        {
            ...
            // Moving the Camera
            if((wParam & MK_LBUTTON) && !mouseClickedUIButton)
            {
            ...
            // Mouse Hovering
            else if(!mouseClickedUIButton)
            {
                // Get the quadrant
                uint quadrant = 0;
                if(last_x < m_iWidth/2)
                {
                    if(last_y > m_iHeight/2)
                        quadrant = 2;
                }
                else
                {
                ...
                // Process it
                pView->MouseHoveringAt(last_x, last_y, quadrant);
            }
        }
        return 0;
    default:
